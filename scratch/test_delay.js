...
async function run() {
    const testWords = ['포도', '딸기', '수박', '오렌지', '레몬'];
    const outputDir = path.join(__dirname, 'test_images_delay');
    if (!fs.existsSync(outputDir)) {
        fs.mkdirSync(outputDir, { recursive: true });
    }

    console.log(`Starting delay test download of ${testWords.length} images...`);
    let success = 0;
