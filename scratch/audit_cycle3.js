...
function auditCycle3() {
    console.log('--- AUDITING CYCLE 3 VOCABULARY & AUDIO & ILLUSTRATION ---');
    const vocab = loadVocabularyData();
    const cycle3 = vocab['beginner_cycle_3'];
    
    if (!cycle3) {
        console.error("Cycle 3 data not found!");
        process.exit(1);
    }
    
    console.log(`Auditing ${cycle3.length} words in beginner_cycle_3...`);
    ...
    cycle3.forEach((word, index) => {
        const ref = `[beginner_cycle_3][Word #${index + 1}: ${word.kr || 'MISSING'}]`;
    ...
    console.log(`\nAudit finished for Cycle 3.`);
    console.log(`- Total words: ${cycle3.length}`);
    ...
    if (issues.length > 0) {
        ...
        console.log('No spelling, translation, audio, or metadata errors found in Cycle 3!');
    }
}
