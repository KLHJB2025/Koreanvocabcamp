...
console.log("Words in Cycle 3 with preloaded illustrations:");
cycle3.forEach(word => {
    if (downloadedSet.has(word.kr)) {
        console.log(`- ${word.kr}: [Local File: public/illustrations/words/${word.kr}.jpg] (ZH: "${word.zh}", EN: "${word.en}")`);
    } else if (word.illustrationUrl) {
        console.log(`- ${word.kr}: [Direct URL: ${word.illustrationUrl}] (ZH: "${word.zh}", EN: "${word.en}")`);
    }
});
