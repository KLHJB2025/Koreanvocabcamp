...
function auditCycle4() {
    console.log('--- AUDITING CYCLE 4 VOCABULARY & AUDIO & ILLUSTRATION ---');
    const vocab = loadVocabularyData();
    const cycle4 = vocab['beginner_cycle_4'];
    
    if (!cycle4) {
        console.error("Cycle 4 data not found!");
        process.exit(1);
    }
    
    console.log(`Auditing ${cycle4.length} words in beginner_cycle_4...`);
    ...
    cycle4.forEach((word, index) => {
        const ref = `[beginner_cycle_4][Word #${index + 1}: ${word.kr || 'MISSING'}]`;
    ...
    console.log(`\nAudit finished for Cycle 4.`);
    console.log(`- Total words: ${cycle4.length}`);
    ...
    if (issues.length > 0) {
        ...
        console.log('No spelling, translation, audio, or metadata errors found in Cycle 4!');
    }
}
