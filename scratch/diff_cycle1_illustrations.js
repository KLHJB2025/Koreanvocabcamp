...
const missing = [];
cycle1.forEach(word => {
    if (!downloaded.has(word.kr)) {
        missing.push(word);
    }
});

console.log(`Total words in Cycle 1: ${cycle1.length}`);
console.log(`Missing from downloaded set: ${missing.length}`);
missing.forEach(w => {
    console.log(`- ${w.kr} (${w.pos}): EN: "${w.en}", ZH: "${w.zh}"`);
});
