                ...
                >
                    ...
                    <div className="space-y-6">
                        <div className="flex items-center gap-4">
                            ...
                            <span className="font-bold text-foreground/70 text-lg">{t('auth.pickUp')}</span>
                        </div>
                        <div className="flex items-center gap-4">
                            <div className="w-10 h-10 rounded-full bg-white shadow-sm flex items-center justify-center text-primary">
                                <ArrowRight size={20} />
                            </div>
                            <span className="font-bold text-foreground/70 text-lg">{t('auth.checkRank')}</span>
                        </div>
                    </div>
                </motion.div>
            </div>
            ...
            <div className="flex flex-col items-center justify-center p-8 lg:p-24 bg-white dark:bg-card">
                ...
                >
                    <div className="text-center lg:text-left">
                        <h2 className="text-3xl font-bold mb-2">{t('auth.accountLogin')}</h2>
                        <p className="text-foreground/40 font-medium tracking-tight">{t('auth.credentialsDesc')}</p>
                    </div>
                    ...
                    <div className="space-y-4">
                        ...
                        <div className="relative flex items-center gap-4 py-2">
                            ...
                            <span className="text-[10px] font-bold text-foreground/30 uppercase tracking-widest">{t('auth.emailLogin')}</span>
                            <div className="flex-1 border-t border-border" />
                        </div>
                        ...
                        <form onSubmit={handleEmailLogin} className="space-y-6">
                            ...
                            <div className="space-y-2">
                                <label className="text-xs font-bold text-foreground/40 uppercase tracking-widest ml-1">{t('auth.emailLabel')}</label>
                            ...
                            <div className="space-y-2">
                                <div className="flex justify-between items-center ml-1">
                                    <label className="text-xs font-bold text-foreground/40 uppercase tracking-widest">{t('auth.passwordLabel')}</label>
                                    <Link href="/forgot-password" className="text-[10px] font-bold text-primary uppercase hover:underline">{t('auth.forgot')}</Link>
                                </div>
                                <div className="relative">
                            ...
                            >
                                ...
                                {loading ? t('auth.authenticating') : t('auth.signInNow')}
                            </button>
                        </form>
                    </div>
                    ...
                    <p className="text-center text-sm text-foreground/40 font-medium">
                        {t('auth.noAccount')}{' '}
                        <Link href="/signup" className="text-primary font-bold hover:underline">{t('auth.createAccount')}</Link>
                    </p>
                </motion.div>
            </div>
        </div>
